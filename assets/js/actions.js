var page_actions = function () {
    ...
    /*$(".section-item-remove").on("click", function (e) {
    ...
    /* Remove Documents Items */
    $(".item-remove").on("click", function () {

        var Id = $(this).attr('id');
        var Url = $(this).attr('data-url') || null;
        var uType = $(this).attr('data-type') || null;
        var pClass;
        ...
        $(this).parents(pClass).fadeOut(400, function () {

            // Delete
            $.ajax({
                method: "POST",
                url: Url,
                data: {id: Id, type: uType},
                success: function (data) {
    ...
    /* $(".slider-item-remove").on("click", function (e) {
    ...
    /* Remove News Items */
    /*$(".news-item-remove").on("click", function (e) {
    ...
    /*$("#gallery-toggle-items").on("click", function () {
     ...
     var hedit = $('.editSlider');

     if (hedit.hasClass("hide")) {
     hedit.removeClass("hide");
     hedit.addClass("show");
     } else {
     hedit.removeClass("show");
     hedit.addClass("hide");
     }
    ...
    $(".lockscreen-box .user_signin").on("click", function () {
        $(".sign-in").show();
        $(this).remove();
        $(".user").hide().find("img").attr("src", "assets/admin/images/users/no-image.jpg");
        $(".user").show();
        return false;
    });
    /* END LOCK SCREEN */
...
$("#site_close, #site_popup").on('click', function () {
...
var text_max = 160;
var text_max2 = 60;
$('#meta_keywords_sub').html(text_max + ' karakter kaldı');
$('#meta_desc_sub').html(text_max + ' karakter kaldı');
$('#meta_title_sub').html(text_max2 + ' karakter kaldı');

$('#meta_keywords').keyup(function () {
    var text_length = $('#meta_keywords').val().length;
    var text_remaining = text_max - text_length;

    $('#meta_keywords_sub').html(text_remaining + ' karakter kaldı');
});

$('#meta_desc').keyup(function () {
    var text_length = $('#meta_desc').val().length;
    var text_remaining = text_max - text_length;

    $('#meta_desc_sub').html(text_remaining + ' karakter kaldı');
});
...
$('#meta_title').keyup(function () {
    ...
    $('#meta_title_sub').html(text_remaining + ' karakter kaldı');
});

function addNewInput(url) {
    $("#btn_" + url).hide();
    $("#new_" + url).show();
}
...
/* Populate Dropdown */
function getPopId(id, url) {
    $.ajax({
        type: "GET",
        url: "/admin/co_destinations/" + url + "/" + id,
        success: function (data) {

            // Add Seas and Country
            $("#btn_" + url).show();
