...
<div class="page-content">
	...
	<ul class="breadcrumb">
		...
		<li class="active">İçerik Güncelle</li>
	</ul>
	<!-- END BREADCRUMB -->
	...
	<div class="page-content-wrap">
		...
		<div class="col-md-5">
			<div class="panel panel-colorful">
				<form id="validate" role="form" action="<?php echo base_url();?>admin/co_pages/editSectionData/<?=$get_data['id'];?>" method="post" enctype="multipart/form-data">
					...
					<div class="panel-body">
								...
								<?php foreach ($categories as $cat): ?>
									<?php if ($cat['children']) { ?>
										<option value="<?=$cat['id']?>"
											...
											<?php if ($cat['id'] == $get_data['sub_category_id']) { echo "selected"; } ?>>
											<?php echo $cat['section_name']; ?>
										</option>
											<?php foreach ($cat['children'] as $alt_cat) { ?>
												<option value="<?php echo $alt_cat['id']; ?>" <?php if ($alt_cat['id'] == $get_data['sub_category_id']) { echo "selected"; } ?>>&nbsp;&nbsp;- <?php echo stripslashes($alt_cat['section_name']); ?></option>
											<?php if($alt_cat['children']) {?>
												<?php foreach ($alt_cat['children'] as $alt_cat2) { ?>
												<option value="<?php echo $alt_cat2['id']; ?>" <?php if ($alt_cat2['id'] == $get_data['sub_category_id']) { echo "selected"; } ?>>&nbsp;&nbsp;&nbsp;-- <?php echo stripslashes($alt_cat2['section_name']); ?></option>
												<?php } ?>
											<?php } ?>
											<?php } ?>
									<?php } else { ?>
										<?php if($cat['section_url'] != '/') {?>
										<option value="<?php echo $cat['id']; ?>" <?php if ($cat['id'] == $get_data['sub_category_id']) { echo "selected"; } ?>>
						...
						<div class="form-group">
							<label>İçerik</label>
							<textarea name="content" class="summernote" placeholder="İçerik"><?=$get_data['content'];?></textarea>
						</div>
						<div class="form-group">
							<label>Seo Açıklama</label>
							<input name="seo_desc" type="text" class="form-control" placeholder="Seo Açıklama (Max:160 karakter)" value="<?=$get_data['seo_desc'];?>">
						</div>
						<div class="form-group">
							<label>Seo Anahtar Kelimeler</label>
							<input name="seo_keywords" type="text" class="form-control" placeholder="Seo Anahtar Kelimeler (Max:160 karakter)" value="<?=$get_data['seo_keywords'];?>">
						</div>
						<div class="form-group">
